...
Bitmap *read_header() {
    ...
    unsigned char temp[BMP_HEADER_SIZE_OFFSET];

    fread(temp, sizeof(unsigned char), BMP_HEADER_SIZE_OFFSET, stdin);
    fread(&(bmp->headerSize), sizeof(int), 1, stdin);

    bmp->header = malloc(bmp->headerSize);
    memcpy(bmp->header, temp, sizeof(temp));
    memcpy(&(bmp->header[BMP_HEADER_SIZE_OFFSET]), &(bmp->headerSize), sizeof(int));
    fread(&(bmp->header[BMP_HEADER_SIZE_OFFSET + 4]), sizeof(unsigned char), bmp->headerSize - BMP_HEADER_SIZE_OFFSET - 4, stdin);

    memcpy(&(bmp->width), &(bmp->header[BMP_WIDTH_OFFSET]), sizeof(int));
    memcpy(&(bmp->height), &(bmp->header[BMP_HEIGHT_OFFSET]), sizeof(int));
...
void scale(Bitmap *bmp, int scale_factor) {
    ...
    file_size = 3 * width * height;
    file_size += bmp->headerSize;
